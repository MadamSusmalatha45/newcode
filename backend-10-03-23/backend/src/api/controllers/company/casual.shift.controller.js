...
exports.updateStatus = async (req, res, next) => {
  try {
    if(req.body.status){
      ...
      var getUser = await User.findOne({_id : req.body.assignedUser})
        ...
        if(req.body.assignedUser){
          const shiftUpdate = await Shift.updateOne({_id : req.params.shiftId}, req.body, async function(err, data){
            if(err){
              ...
              let userShift = await UserShift.findOne({casualShiftId : req.params.shiftId},{assignedUser : req.body.assignedUser})
              ...
              req.status(201).send({code : 201, message : 'Casual shift status updated successfully.'});
            }
          });
        }
        else{
          req.status(500).send({code : 500, message : 'Assigned User is required', data : {}});
        }
      }
      ...
      req.status(400).send({code : 400, message : 'Status is required', data : {}});
    }
  } catch (error) {
    next(error);
  }
...
exports.listReport = async (req, res, next) => {
  try {
    const  {startDate, endDate, siteId, page = 1, perPage = 25} =req.body;
...
exports.listUser = async (req, res, next) => {
  try {
    ...
    const shifts = await Shift.find({userId : req.user._id, status : 'Approved' }, { shiftType: 0, price: 0, shiftCode: 0, shiftRecurrence: 0, intrestedUsers: 0, assignedUser: 0})
      .populate([
        {
          path: 'siteId',
          model: 'Site',
    ...
    var data = [];
    transformedShifts.forEach((item) => {
