  ...
  try{
    ...
    if(shift){
      if(shift.clockInTime === null){
        const updateShift = await Shift.updateOne({_id:id},{clockInTime:req.body.clockInTime}, async function(err, update){
          if(err){
            next(err)
          }
          else{
  ...
  try{
    if(shift){
      if(shift.clockOutTime === null){
        const updateShift = await Shift.updateOne({_id:id},{clockOutTime:req.body.clockOutTime}, async function(err, update){
          if(err){
            next(err)
          }
          else{
