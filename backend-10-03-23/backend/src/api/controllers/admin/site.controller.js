...
exports.getLiveView = async (req, res, next) => {
    ...
    if (req.query.date) {
        ...
        clockins = await Shift.find({ siteId: req.locals.site._id, 'inOut': 'IN', 'createdAt': { '$gte': startDate, '$lte': endDate }, })
        liveView.shiftLogs = await ShiftLogs.find({ shiftId: { $in: clockins } }).count()
        // clockins.map(e=>{})
        breakReport = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^break/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
        ...
        fire = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^fire/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
        ...
        injury = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^injury/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
        ...
        bomb = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^bomb/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
        ...
        hazard = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^hazard/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
        ...
        vandal = await Reports.aggregate([
            {
                $match: { siteId: req.locals.site._id, name: { $regex: /^vandalism/i }, 'createdAt': { '$gte': new Date(startDate), '$lte': new Date(endDate) } }
            },
            {
                $lookup: {
                    "from": "reports",
...
exports.getOverview = async (req, res, next) => {
    ...
    var timeSheets = null;
    var resources = await Site.findOne({_id : req.locals.site });
    var tendencies = null;
    var shiftLogs = null;
    var tasks = null;
    var roasterShifts = [];
    ...
    if(startDate &&  endDate){
        totalShifts = await Shift.find({siteId:req.locals.site, startDate : {$gte : startDate},endDate : {$lte : endDate}}).populate('breaks');
        casualShifts = await Shift.find({siteId:req.locals.site, casualShiftId : {$ne : null}, startDate : {$gte : startDate},endDate : {$lte : endDate}});
        roasterShifts = await Shift.find({siteId:req.locals.site, rosterId : {$ne : null}, startDate : {$gte : startDate}, endDate : {$lte : endDate}})
        allShifts = await Shift.find({siteId:req.locals.site, totalFailedCheckIn : null, startDate : {$gte : startDate},endDate : {$lte : endDate}});
        allReports = await Report.find({siteId:req.locals.site, createdAt : {$gte : startDate, $lte : endDate}});
        ...
        timeSheets = await Timesheet.find({siteId : req.locals.site,createdAt : {$gte : startDate, $lte : endDate}});
        ...
        alarms = await Alarm.find({sites : req.locals.site, dueDate : {$gte : startDate, $lte : endDate}});
    }
    else if(startDate && !endDate){
        totalShifts = await Shift.find({siteId:req.locals.site, startDate : {$gte : startDate}}).populate('breaks');
        casualShifts = await Shift.find({siteId:req.locals.site,casualShiftId : {$ne : null}, startDate: {$ne : null}, endDate: {$ne : null}, startDate : {$gte : startDate}});
        roasterShifts = await Shift.find({siteId:req.locals.site,rosterId : {$ne : null}, startDate : {$gte : startDate}})
        allShifts = await Shift.find({siteId:req.locals.site, totalFailedCheckIn : null, startDate : {$gte : startDate}});
        allReports = await Report.find({siteId:req.locals.site,createdAt : {$gte : startDate}});
        licenses = await License.find({expiryDate : {$gte : startDate}});
        notifications =  await notificationModel.find({startDate : {$gte : startDate}});
        timeSheets = await Timesheet.find({siteId : req.locals.site,createdAt : {$gte : startDate}});
        ...
        alarms = await Alarm.find({sites : req.locals.site, dueDate : {$gte : startDate}});
    }
    else if(!startDate && endDate){
        totalShifts = await Shift.find({siteId:req.locals.site, endDate : {$gte : endDate}}).populate('breaks');
        casualShifts = await Shift.find({siteId:req.locals.site, casualShiftId : {$ne : null}, startDate: {$ne : null}, endDate: {$ne : null}, endDate : {$gte : endDate}});
        roasterShifts = await Shift.find({siteId:req.locals.site,rosterId : {$ne : null}, endDate : {$lte : endDate}})
        allShifts = await Shift.find({siteId:req.locals.site, totalFailedCheckIn : null, endDate : {$gte : endDate}});
        allReports = await Report.find({siteId:req.locals.site, createdAt : {$lte : endDate}});
        licenses = await License.find({expiryDate : {$lte : endDate}});
        notifications =  await notificationModel.find({endDate : {$lte : endDate}});
        timeSheets = await Timesheet.find({siteId : req.locals.site,createdAt : { $lte : endDate}});
        ...
        alarms = await Alarm.find({sites : req.locals.site, dueDate : { $lte : endDate}});
    }
    else{
        totalShifts = await Shift.find({siteId:req.locals.site}).populate('breaks');
        casualShifts = await Shift.find({siteId:req.locals.site,casualShiftId : {$ne : null}, startDate: {$ne : null}, endDate: {$ne : null}});
        roasterShifts = await Shift.find({siteId:req.locals.site, rosterId : {$ne : null},})
        allShifts = await Shift.find({siteId:req.locals.site, totalFailedCheckIn : null});
        allReports = await Report.find({siteId:req.locals.site});
        licenses = await License.find({ expiryDate : {$lt : new Date()}});
        notifications =  await notificationModel.find({});
        timeSheets = await Timesheet.find({siteId : req.locals.site});
        ...
        alarms = await Alarm.find({sites : req.locals.site});
    }
    ...
    var fireAlarmCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abe3'){
            return item
        }
    }).length;

    var tresPassingReportedCount = allReports.map((item) => {
        if(item.reportTypeId === '63de643044b3381cb833abe5'){
            return item
        }
    }).length;

    var hazardReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abe6'){
            return item
        }
    }).length;

    var vandalismReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abe7'){
            return item
        }
    }).length;
    ...
    var incidentReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abe4'){
            return item
        }
    }).length;

    var bombThreatReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abe9'){
            return item
        }
    }).length;

    var injuryReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abea'){
            return item
        }
    }).length;

    var periodicReportedCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abeb'){
            return item
        }
    }).length;

    var suspiciousActivityCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abec'){
            return item
        }
    }).length;

    var lostPropertyCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abed'){
            return item
        }
    }).length;

    var theftCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abee'){
            return item
        }
    }).length;

    var firstAidCount = allReports.map((item) => {
        if(item.Report === '63de643044b3381cb833abef'){
            return item
        }
    }).length;

    var outOutDateCount = allReports.map((item) => {
    ...
    var timeSubmitted = timeSheets.map((item) => {
        if(item.statusId === '62a8eb3bfc9fbe1ba86e134d'){
            return item
        }
    }).length;

    var timePending = timeSheets.map((item) => {
        if(item.statusId === '62e814c2049bd2e0a191399f'){
            return item
        }
    }).length;

    var timeApproved = timeSheets.map((item) => {
    ...
    if(req.user.login_as  === 'CLIENT' || req.user.login_as === 'COMPANY_CLIENT' || req.user.login_as === 'COMPANY_USER'){
        dynamics = await Dynamic.find({siteId : req.locals.site,role : {$in : ['CLIENT','COMPANY_CLIENT','COMPANY_USER']} })
        .populate([
            {
                path : 'reportTypeId',
                model : 'AllReportType'
        ...
        dynamics = await Dynamic.find({siteId : req.locals.site,role : 'SUPERADMIN'} )
        .populate([
            {
                path : 'reportTypeId',
                model : 'AllReportType'
    ...
    var data = {
        ...
        compliance :  [
            ...
            {title : 'Out Of Date Inductions', count : outOutDateCount ? outOutDateCount.length : 0},
