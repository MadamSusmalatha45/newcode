...
exports.remove = async(req, res, next) => {
  ...
  if(choice){
       let questions = await Question.find({choices : {$ne : req.params.choiceId}});
       if(questions){
        for(var i = 0; i < questions.length; i++){
          var arr = questions[i].choices;
          var filteredArray = arr.filter(function(e) { return e !== req.params.choiceId })
          let updateQuestion = await Question.updateOne({_id : questions[i]._id},{choices : filteredArray});
        }
